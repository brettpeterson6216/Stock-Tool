...
function productionReadiness(env = process.env, databaseReady = true, catalogReady = true) {
  ...
  const checks = {
    ...
    billing: [env.STRIPE_SECRET_KEY, env.STRIPE_WEBHOOK_SECRET, env.STRIPE_PRICE_MONTHLY, env.STRIPE_PRICE_ANNUAL].every(present) &&
      (typeof catalogReady === "object" ? catalogReady?.ready === true : catalogReady !== false),
