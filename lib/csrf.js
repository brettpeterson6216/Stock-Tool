...
function validateCsrf(req, res, next) {
  ...
  const sessionBuffer = typeof sessionToken === "string" ? Buffer.from(sessionToken) : null;
  const headerBuffer  = typeof headerToken === "string" ? Buffer.from(headerToken) : null;
  const valid = sessionBuffer && headerBuffer &&
    sessionBuffer.length === headerBuffer.length &&
