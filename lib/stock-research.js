...
const OPTIONAL_RESEARCH_BUDGET_MS = 5500;
...
async function withinBudget(promise, timeoutMs, label) {
  let timer;
  try {
    return await Promise.race([
      promise,
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`${label} exceeded the response-time budget.`)), timeoutMs);
      }),
    ]);
  } finally {
    clearTimeout(timer);
...
function reportedQuarterlyEps(companyFacts) {
  const direct = quarterlySeries(companyFacts, CONCEPTS.dilutedEps).map(row => ({
    period: row.end,
    quarter: row.fp || null,
    year: row.fy || Number(String(row.end).slice(0, 4)),
    actual: number(row.val),
    actualAsOf: row.filed || null,
    actualBasis: "SEC-reported diluted GAAP EPS",
    derived: false,
  }));
  const byPeriod = new Map(direct.map(row => [row.period, row]));

  // Most 10-K filings report full-year EPS but not a standalone Q4 fact.
  // Reconstruct Q4 only when the same filing year has a complete Q1-Q3 set.
  for (const annual of annualSeries(companyFacts, CONCEPTS.dilutedEps)) {
    if (!annual.start || !annual.end || byPeriod.has(annual.end) || !finite(annual.val)) continue;
    const firstThree = direct
      .filter(row => row.period >= annual.start && row.period < annual.end && finite(row.actual))
      .sort((a, b) => a.period.localeCompare(b.period));
    if (firstThree.length !== 3) continue;
    const q4 = Math.round((Number(annual.val) - firstThree.reduce((sum, row) => sum + Number(row.actual), 0)) * 100) / 100;
    if (!finite(q4)) continue;
    byPeriod.set(annual.end, {
      period: annual.end,
      quarter: "Q4",
      year: annual.fy || Number(String(annual.end).slice(0, 4)),
      actual: q4,
      actualAsOf: annual.filed || null,
      actualBasis: "Derived from SEC annual diluted GAAP EPS less reported Q1-Q3",
      derived: true,
    });
  }
...
function deriveFundamentals(companyFacts, price, finnhub = {}, market = {}) {
  ...
  const reportedQuarterlyActuals = reportedQuarterlyEps(companyFacts).map(row => ({
    period: row.period,
    actual: row.actual,
  }));
  ...
  const rawReferenceEpsSelection = selectReferenceEps({
    ...
    earnings: reportedQuarterlyActuals.length ? reportedQuarterlyActuals : (finnhub.earnings || []),
  });
  ...
  const fields = {
    ...
    dcfUpside: { value: modeledUpside, source: "ImpliedLens normalized earnings-value model", asOf },
    impliedGrowthGap: { value: impliedGrowthGap, source: "ImpliedLens price-expectations model", asOf },
    bearDownside: { value: bearDownside, source: "ImpliedLens conservative earnings-value model", asOf },
  };
  return {
    values,
    fields,
...
function deriveEarnings(companyFacts, finnhub = {}) {
  const providerRows = Array.isArray(finnhub.earnings) ? finnhub.earnings : [];
  const providerByPeriod = new Map(
    providerRows
      .filter(row => /^\d{4}-\d{2}-\d{2}$/.test(String(row?.period || "")))
      .map(row => [String(row.period), row])
  );
  const reported = reportedQuarterlyEps(companyFacts);
  if (reported.length) {
    return reported.slice(0, 20).map(row => {
      const providerRow = providerByPeriod.get(row.period) || null;
      const estimate = number(providerRow?.estimate);
      const consensusActual = number(providerRow?.actual);
      const tolerance = Math.max(0.02, Math.abs(Number(row.actual)) * 0.025);
      const comparable = finite(estimate) && finite(consensusActual)
        && Math.abs(Number(row.actual) - Number(consensusActual)) <= tolerance;
      const surprise = comparable ? Number(row.actual) - Number(estimate) : null;
      const surprisePercent = comparable && Number(estimate) !== 0
        ? (surprise / Math.abs(Number(estimate))) * 100
        : null;
      return {
        ...row,
        estimate,
        ...
        surprisePercent,
        comparable,
        comparisonStatus: comparable ? "comparable" : estimate === null ? "estimate-unavailable" : "basis-mismatch",
        comparisonNote: comparable
          ? "SEC-reported GAAP EPS aligns with the provider's consensus-compatible actual."
          : estimate === null
            ? "No matching analyst consensus estimate is available for this fiscal period."
            : "SEC-reported GAAP EPS differs from the provider's consensus-compatible actual, so no beat/miss is calculated.",
        source: providerRow
          ? "SEC EDGAR XBRL actuals + Finnhub consensus estimates"
          : "SEC EDGAR XBRL actuals",
        asOf: row.actualAsOf,
      };
    });
  }
  return providerRows.slice(0, 20).map(row => ({
    period: row.period || null,
    quarter: row.quarter || null,
    year: row.year || null,
    actual: number(row.actual),
    estimate: number(row.estimate),
    consensusActual: number(row.actual),
    surprise: number(row.surprise),
    surprisePercent: number(row.surprisePercent),
    comparable: true,
    comparisonStatus: "provider-only",
    comparisonNote: "SEC quarterly GAAP EPS is unavailable; this row uses the provider's consensus-compatible earnings basis.",
    actualBasis: "Finnhub consensus-compatible EPS (not SEC-reported GAAP EPS)",
    actualAsOf: row.period || null,
...
async function buildResearchBundle(ticker, options = {}) {
  ...
  const [priceResult, factsResult, finnhubResult] = await Promise.allSettled([
    ...
    withinBudget(loadCompanyFacts(normalized), OPTIONAL_RESEARCH_BUDGET_MS, "SEC company evidence"),
    withinBudget(loadFinnhubResearch(normalized), OPTIONAL_RESEARCH_BUDGET_MS, "Consensus and provider metrics"),
  ]);
...
/* ═══════════════════════════════════════════════════════════════════════════
   Release a company's SEC facts from the cache.

   loadCompanyFacts caches the whole companyfacts document per CIK for thirty
   minutes. That is right for the app: a visitor analysing a ticker will touch
   it several times in a session, and refetching a 20MB document each time
   would be worse than holding it.

   It is wrong for anything that reads a company ONCE and is done — the stock
   landing pages derive their figures and never look at the raw facts again.
   Those callers accumulated documents they had no further use for until the
   process hit V8's heap limit and aborted (exit 134, in production, from a
   warm-up that read 103 companies inside one TTL window).

   So: a caller that knows it is finished with a company says so, and the
   several megabytes go back immediately rather than in half an hour. The
   derived figures it already extracted are unaffected — they are plain
   numbers on an object this does not touch.
   ═══════════════════════════════════════════════════════════════════════════ */
async function forgetCompanyFacts(ticker) {
  try {
    const company = await resolveCompany(ticker);
    return cache.delete(`facts:${company.cik}`);
  } catch (_) {
    /* Never resolved, so nothing is held. */
    return false;
  }
}
