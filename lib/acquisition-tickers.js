...
function buildSitemapXml(baseUrl = "https://impliedlens.com") {
  ...
  /* Every lesson gets its own entry. They were unreachable to a crawler
     entirely before this — the whole teaching content of the site lived behind
     a query string inside the app. */
  const { lessons } = require("../public/lessons-data.js");
  for (const lesson of lessons) staticPages.push([`/learn/${lesson.slug}`, "monthly", "0.7"]);
  const entries = [
