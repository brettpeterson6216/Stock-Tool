...
async function reconcileEffectivePlan(user) {
  ...
  if (localPlan === "pro" || localPlan === "trial" || (!user?.stripe_subscription_id && !user?.stripe_customer_id) || !stripe) {
    return localPlan;
  }
  try {
    let subscription;
    if (user.stripe_subscription_id) {
      ...
      const subscriptions = await stripe.subscriptions.list({ customer: user.stripe_customer_id, status: "all", limit: 10 });
      const priority = ["trialing", "active", "past_due"];
      subscription = priority.map(status => subscriptions.data.find(item => item.status === status)).find(Boolean)
        || subscriptions.data[0];
    }
    if (!subscription) return localPlan;
    const plan = subscriptionStatusToPlan(subscription.status);
    const trialEnd = subscription.trial_end ? new Date(subscription.trial_end * 1000).toISOString() : null;
    if (plan !== user.plan || trialEnd !== (user.trial_ends_at || null) || subscription.id !== user.stripe_subscription_id) {
      await db.execute({
        sql: "UPDATE users SET plan = ?, trial_ends_at = ?, stripe_subscription_id = ? WHERE id = ?",
        args: [plan, trialEnd, subscription.id, user.id],
      });
      user.plan = plan;
