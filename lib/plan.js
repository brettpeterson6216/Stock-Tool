...
const { PRODUCT_CONFIG } = require("./product-config");

const stripe = STRIPE_SECRET_KEY ? Stripe(STRIPE_SECRET_KEY, { timeout: 5000, maxNetworkRetries: 0 }) : null;

// Compatibility exports retained for route and test consumers. The values are
// owned by the application-level product configuration.
const FREE_DAILY_LIMIT  = PRODUCT_CONFIG.analysisLimits.registeredFreeDaily;
const GUEST_DAILY_LIMIT = PRODUCT_CONFIG.analysisLimits.guestDaily;
...
function subscriptionStatusToPlan(status) {
...
function subscriptionAllowsAccess(subscription) {
  return subscriptionStatusToPlan(subscription?.status) !== "free";
}
...
function pickBestSubscription(subscriptions) {
  const rows = (subscriptions || []).filter(Boolean);
  if (!rows.length) return null;
  const priority = ["trialing", "active", "past_due", "incomplete", "unpaid", "canceled"];
...
async function findStripeSubscriptionForUser(user) {
  ...
  for (const customerId of customerIds) {
    subscriptions.push(...await listSubscriptionsForCustomer(customerId));
  }

  if (user.email && !subscriptions.some(subscriptionAllowsAccess)) {
    const customers = await stripe.customers.list({ email: String(user.email).toLowerCase(), limit: 10 });
    for (const customer of customers.data || []) {
      if (!customer?.id || customer.deleted || customerIds.has(customer.id)) continue;
      customerIds.add(customer.id);
      subscriptions.push(...await listSubscriptionsForCustomer(customer.id));
    }
  }
  ...
  return {
    subscription,
    customerId: subscriptionCustomerId(subscription) || user.stripe_customer_id || [...customerIds][0] || null,
  };
}

async function reconcileEffectivePlan(user) {
  const localPlan = getEffectivePlan(user);
  if (localPlan === "pro" || localPlan === "trial" || !stripe || (!user?.stripe_subscription_id && !user?.stripe_customer_id && !user?.email)) {
    return localPlan;
  }
  try {
    const match = await findStripeSubscriptionForUser(user);
    const subscription = match?.subscription;
    if (!subscription) return localPlan;
    const plan = subscriptionStatusToPlan(subscription.status);
    const trialEnd = subscription.trial_end ? new Date(subscription.trial_end * 1000).toISOString() : null;
    const customerId = match.customerId || user.stripe_customer_id || null;
    if (
      plan !== user.plan ||
      ...
      await db.execute({
        ...
        args: [plan, trialEnd, customerId, subscription.id, user.id],
      });
      user.plan = plan;
    ...
    console.error("[entitlement] Stripe reconciliation failed:", e.message);
    return localPlan;
  }
}
...
async function requirePro(req, res, next) {
  ...
  try {
    const r = await db.execute({
      sql:  "SELECT id, email, plan, trial_ends_at, stripe_customer_id, stripe_subscription_id FROM users WHERE id = ?",
      args: [req.session.userId],
    });
    const user = r.rows[0];
    if (!user) return res.status(401).json({ error: "User not found.", requiresLogin: true });
    const plan = await reconcileEffectivePlan(user);
...
async function checkAnalysisLimit(req, res, next) {
  ...
  if (req.session.userId) {
    ...
    plan = await reconcileEffectivePlan(row);
    if (plan === "pro" || plan === "trial") {
      res.setHeader("X-Plan", plan);
      return next();
    }
...
module.exports = {
  getEffectivePlan, reconcileEffectivePlan, subscriptionStatusToPlan, requirePro, checkAnalysisLimit,
  guestIdMiddleware, normalizeTicker,
