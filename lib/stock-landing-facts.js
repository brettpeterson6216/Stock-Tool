...
const { buildResearchBundle, forgetCompanyFacts } = require("./stock-research");
...
/* A HARD ceiling on how many distinct tickers this process will hydrate in an
   ...
   worse one. */
/* The cap is about memory, not politeness to the providers.

   buildResearchBundle pulls the whole SEC companyfacts document — tens of
   megabytes parsed — and stock-research caches it for thirty minutes against
   a 300-ENTRY limit that does not count bytes. Reading the published set
   inside one TTL window held about a hundred of those at once and V8 aborted
   on its heap limit (exit 134).

   Two things keep that from recurring, and the second is the real one:

     - this hourly cap, so a crawler walking all 103 sitemap URLs in one pass
       cannot queue the whole set; and
     - forgetCompanyFacts below, which hands the document back the moment the
       figures are extracted. Peak is then MAX_INFLIGHT documents — two — no
       matter how many tickers are hydrated, because none of them is retained.

   With the retention gone the cap is a belt to the braces rather than the
   only thing standing between a crawl and an abort, which is why it can sit
   at a number that actually serves the pages. */
const MAX_HYDRATIONS_PER_HOUR = 40;
...
function refresh(ticker, nameHint) {
  ...
  buildResearchBundle(ticker, { range: "5y", interval: "1mo" })
    ...
    .finally(() => {
      inflight -= 1;
      /* The summary above is plain numbers. The document it came from is not,
         and this page will never read it again. In the .finally so it runs on
         the failure path too, where a partially-populated cache entry is
         exactly as heavy and exactly as useless. */
      forgetCompanyFacts(ticker).catch(() => {});
    });
}
