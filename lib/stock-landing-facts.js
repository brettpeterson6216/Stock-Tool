...
function summarize(bundle, ticker) {
  ...
  const name = bundle?.company?.name || bundle?.profile?.name || ticker;
  return {
    ticker,
    groups: GROUPS
...
function refresh(ticker) {
  ...
  buildResearchBundle(ticker, { range: "5y", interval: "1mo" })
    .then(bundle => {
      const summary = summarize(bundle, ticker);
...
function landingFacts(ticker) {
  const entry = cache.get(ticker);
  if (!entry) { refresh(ticker); return null; }
  if (Date.now() - entry.at > TTL_MS) refresh(ticker);
