...
test("generated brand assets spell the brand one word too", () => {
  /* logo.svg is generated by scripts/build-brand.js and carries the brand name
     in its <title> and aria-label -- which is what a screen reader announces
     for the logo, and what shows in a tooltip. It said "Implied Lens".

     The sweep that fixed this everywhere else covered HTML and shipped
     JavaScript. An SVG generated by a build script is neither, so it was the
     one surviving place the brand was still two words. */
  const svg = fs.readFileSync(path.join(ROOT, "public", "logo.svg"), "utf8");
  assert.doesNotMatch(svg, /Implied\s+Lens/i, "logo.svg splits the brand into two words");
  assert.match(svg, /<title>ImpliedLens<\/title>/);

  // And the generator, so regenerating cannot put it back.
  const gen = fs.readFileSync(path.join(ROOT, "scripts", "build-brand.py"), "utf8");
  assert.doesNotMatch(gen, /Implied\s+Lens/i, "the brand generator still writes two words");
});
