...
const PRO_GATE_CONFIG = {
  ...
  dcf: {
    ...
    preview: `<div style="text-align:center;padding:.4rem 0 .2rem;">
      <div style="font-size:11px;color:var(--ink5);letter-spacing:.04em;margin-bottom:.3rem">Intrinsic value estimate</div>
      <div style="font-size:1.9rem;font-family:var(--serif);color:var(--il-gold-ink,var(--gold));font-weight:700;line-height:1">$198.40</div>
      <div style="font-size:11px;color:var(--ink4);margin-top:.25rem">vs market $219.86 · <span style="color:#d96a70">−9.8% overvalued</span></div>
      <div style="margin-top:.65rem;display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;font-size:11px">
...
function updateLimitUI(remaining, limit, plan) {
  ...
  wrap.innerHTML = `
    <span style="font-size:11px;color:var(--text5);">${isGuest ? 'Guest analyses today:' : 'Free analyses today:'}</span>
    <span style="font-size:11px;font-weight:700;color:${color};">${used}/${limit}</span>
...
async function fetchAndRender() {
  ...
  try {
    ...
    if (e.message === 'LIMIT_REACHED') {
      ...
      err.innerHTML=`<span>You've used all <strong>${e.limitData?.limit||5} free analyses</strong> for today. <a href="#" onclick="showUpgradeModal();return false;" style="color:var(--il-gold-ink,var(--gold));font-weight:600;">Upgrade to Pro</a> for unlimited access.</span>`;
    ...
    try {
      ...
      benchEl.innerHTML = `
        <div style="margin-top:1rem;background:var(--t-metric-bg);border:1px solid var(--t-metric-border);border-radius:8px;padding:10px 14px;">
          <div style="font-size:.62rem;color:var(--il-gold-ink,var(--gold));font-family:var(--mono);text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;">vs ${sector||'Sector'} Median</div>
...
function renderMovers(type) {
  ...
  if (!items.length) { el.innerHTML = '<div style="color:var(--text5);font-size:11px;text-align:center;padding:.8rem 0;">Market movers are refreshing</div>'; return; }
...
function loadHomeWatchlist(watchData) {
  ...
  if (!rows.length) { el.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:.9rem 0;color:var(--text5);font-size:11px;">Sign in to see your watchlist</td></tr>'; return; }
      ...
      if(!holders.length) {
        ...
        instTbl.innerHTML = holders.map(h => {
          ...
          return `<tr>
            ...
            <td>${chgStr}</td>
            <td style="font-size:11px;color:var(--text5);">${escapeHtml(h.reportDate||h.date||'—')}</td>
          </tr>`;
        }).join('');
      }
    }
  ...
  try {
    ...
    if (fwdQ.length) {
      fwdHtml = `<div style="background:rgba(185,138,61,.08);border:1px solid rgba(185,138,61,.25);border-radius:8px;padding:10px 14px;margin-bottom:12px;">
        <div style="font-size:.65rem;color:var(--il-gold-ink,var(--gold));font-family:var(--mono);text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;">\u2736 Analyst Forward EPS Estimates</div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
          ${fwdQ.map(q => `<div style="background:var(--t-metric-bg);border:1px solid var(--t-metric-border);border-radius:6px;padding:6px 10px;min-width:90px;">
            ...
            <div style="font-size:11px;color:var(--text5);">$${(q.epsLow||q.epsAvg).toFixed(2)}–$${(q.epsHigh||q.epsAvg).toFixed(2)}</div>
      ...
      summaryHtml = `
        <div style="display:flex;gap:8px;align-items:center;background:var(--t-metric-bg);border:1px solid var(--t-metric-border);border-radius:8px;padding:10px 14px;margin-bottom:12px;flex-wrap:wrap;">
          <div style="flex:1;min-width:180px;">
            <div style="font-size:11px;color:var(--text5);text-transform:uppercase;letter-spacing:.07em;margin-bottom:4px;">${ticker} EPS Beat Rate — Last ${withData.length} Quarters</div>
          ...
          <div style="display:flex;gap:12px;">
            ...
            <span style="font-size:.75rem;color:var(--il-gold-ink,var(--gold));"><strong>${inLine}</strong> In-Line</span>
    ...
    rows.forEach((row, idx) => {
      ...
      tableHtml += `<tr class="earn-expand-row" role="button" tabindex="0" aria-label="Toggle ${row.period||'earnings'} details" onclick="toggleEarnDetail('${detailId}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleEarnDetail('${detailId}')}" style="cursor:pointer;">
        ...
        <td>${badge}</td>
        <td style="text-align:center;color:var(--text5);font-size:11px;" id="chev-${detailId}">›</td>
      </tr>
        ...
        <td colspan="6" style="padding:0;">
          <div style="background:var(--t-metric-bg);border-radius:6px;padding:10px 14px;margin:2px 0 4px 0;border:1px solid var(--t-metric-border);">
            ...
            ${row.surprisePercent != null ? `<div style="margin-top:6px;">
              ...
              <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text5);margin-top:2px;"><span>Miss</span><span>In-Line</span><span>Beat</span></div>
            </div>` : ''}
          </div>
        </td>
      </tr>`;
