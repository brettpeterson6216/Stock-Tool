...
/* The lessons were reachable only at /?view=tool&section=education -- a query
   string inside the app, not a URL. Nothing could link to one, no search engine
   could index one, and reading any of it meant loading the whole application
   first. They are pages now, generated by scripts/build-learn.js from
   public/lessons-data.js, which the in-app panel reads from as well. */
app.get(["/learn", "/learn.html"], (_req, res) => sendPage(res, path.join(__dirname, "public", "learn.html")));
app.get("/learn/:slug", (req, res, next) => {
  // Only the slugs we generated, so a stray path cannot walk the filesystem.
  const slug = String(req.params.slug || "").replace(/\.html$/, "");
  if (!/^[a-z0-9-]{1,40}$/.test(slug)) return next();
  const file = path.join(__dirname, "public", "learn", slug + ".html");
